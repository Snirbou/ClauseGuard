...
| **Deployed and live**: <https://brave-luck-production-ec42.up.railway.app> — web + API + Postgres on Railway, same-origin rewrite over private IPv6, CD on every push to `main` once CI is green | ✅ |
...
| Live deployment (Railway) | `smoke_test.py` 73/73 against the public URL; health `200` with `classifier.mode: model`, `ocr.usable: true`, `database: ok`; session cookie `HttpOnly; Secure; SameSite=lax`; `/api/metrics` `401` unauthenticated |
...
| 1C | Railway deployment (API + web + Postgres), CD on push | — | ✅ live |
