...
/**
 * One clause of the analyzed contract.
 *
 * Progressive disclosure (AC-X04): the default view of an analyzed clause is
 * the badges plus the plain-language summary. The clause's full text and the
...
export default function ClauseCard({ clause }: Props) {
  ...
  const detailsId = `clause-details-${clause.parsed_clause_id}`;

  // Unanalyzed clauses have nothing but their text — show it directly
  // (collapsed when long), as before analysis runs.
  if (!analyzed) {
  ...
  return (
    <li className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 sm:p-5">
      ...
      {/* Default view: the summary only (AC-X04). */}
      ...
      {/* AC-X03: the CTA stays visible for high-risk clauses regardless of
          the details toggle. */}
